...
signal track_data_changed()

func update_midi_track_names(track_names: Array[String]):
	var current_track_name = ""
	var selected_idx: int = %MidiTrackList.selected

	if selected_idx >= 0 and selected_idx < track_names.size():
		current_track_name = track_names[selected_idx]
...
func set_track_data(data: SongTrackData):
	...
	emit_signal("track_data_changed")
...
func _on_midi_track_list_item_selected(index: int) -> void:
	...
	undo_redo.add_do_method(self, "emit_signal", "track_data_changed")
	...
	undo_redo.add_undo_method(self, "emit_signal", "track_data_changed")
